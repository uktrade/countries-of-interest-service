...
def populate_table(schema, table_name, url, stub_data=None):
    ...
    table_name_backup = f'{table_name}_backup'
    try:
        if table_exists(table_name):
            drop_table(table_name_backup)
            rename_table(table_name, table_name_backup)
        ...
        sql = '''create table {table_name} ({schema_str})'''.format(
            table_name=table_name, schema_str=schema_str
        )
        execute_statement(sql, raise_if_fail=True)

        sql = ''' insert into {} values ({}) '''.format(
            table_name, ','.join(['%s' for i in range(len(schema['columns']))])
        )
        values = (
            data['values']
        ...
        output = {
            'status': 500,
            'error': "Failed inserting record into {} table {}".format(
                table_name, error
            ),
        }
        if table_exists:
            rename_table(table_name_backup, table_name)
