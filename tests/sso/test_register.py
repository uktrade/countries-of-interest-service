...
@pytest.fixture
def mock_security():
    with unittest.mock.patch('app.sso.register.Security') as mock:
        yield mock
...
class TestRegisterSsoComponent:
    def test_set_token_sso_client_if_not_role_based(self, mock_app, mock_token_client):
        ...
        assert output == mock_app

    def test_set_role_based_sso_client_if_role_based(
        self, mock_app, mock_role_based_client, mock_security
    ):
        role_based = True
        register_sso_component(mock_app, role_based)
